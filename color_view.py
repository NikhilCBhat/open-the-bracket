...
class ColorView(GameView):
    ...
    def display(self, state: 'GameState'):
        ...
        print(f"{state.current_player.name}'s best number: {state.current_player.best_number}")
        print(f"Move History: {Fore.MAGENTA + ''.join(str(x) for x in state.move_history)}")
        print(f"Remaining integers: {sorted(state.current_player.remaining_digits)}")
        print(f"Remaining operations: {', '.join(sorted(state.current_player.remaining_ops))}")
        print(Fore.CYAN + "-" * 50)
